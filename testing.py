...
if menu == "🏠 Home":
    ...
    if pwd == ADMIN_PASSWORD:
        ...
        with st.sidebar.expander("📁 Update Database Barang (CSV)", expanded=False):
            up_file = st.file_uploader("Pilih file CSV", type=["csv"], key="db_up")
            if up_file is not None:
                if st.button("🚀 Proses Update Database", use_container_width=True):
                    try:
                        with open("database_barang.csv", "wb") as f:
                            f.write(up_file.getbuffer())
                        st.cache_data.clear() # Bersihkan cache data barang
        ...
        # --- 2. KELOLA ANTREAN (FORCE SYNC) ---
        sheet = connect_gsheet()
        if sheet:
            try:
                # Ambil data FRESH dari Google Sheet tanpa cache
                ...
                if len(all_vals) > 1:
                    ...
                    if not pending.empty:
                        for idx, row in pending.iterrows():
                            # Cari index baris asli di GSheet (urutan baris Excel)
                            real_row_idx = df_gs.index[idx] + 2 
                            
                            with st.expander(f"🛠️ KELOLA: {row['Customer']}", expanded=True):
                                # AMBIL DATA TERBARU DARI BARIS TERSEBUT
                                try:
                                    items_list = ast.literal_eval(str(row['Pesanan']))
                                except:
                                    items_list = []

                                with st.form(key=f"form_ultimate_{real_row_idx}"):
                                    st.write("### 1. Daftar Pesanan & Harga")
                                    updated_temp = []
                                    
                                    # Menampilkan barang yang sudah ada
                                    for i, r in enumerate(items_list):
                                        c1, c2, c3, c4, c5, c6 = st.columns([2.5, 0.7, 1, 1.3, 0.7, 0.5])
                                        c1.markdown(f"**{r['Nama Barang']}**")
                                        
                                        # Key sangat unik agar tidak tertukar antar antrean
                                        u_key = f"row{real_row_idx}_item{i}"
                                        
                                        nq = c2.number_input("Qty", value=int(r['Qty']), key=f"q_{u_key}")
                                        ns = c3.text_input("Unit", value=r.get('Satuan','Pcs'), key=f"s_{u_key}")
                                        nh = c4.number_input("Harga", value=float(r['Harga']), key=f"h_{u_key}")
                                        n_pos = c5.number_input("Pos", value=float(i+1), step=0.1, key=f"p_{u_key}")
                                        to_del = c6.checkbox("🗑️", key=f"d_{u_key}")
                                        
                                        updated_temp.append({
                                            "delete": to_del, "pos_index": n_pos, "Nama Barang": r['Nama Barang'], 
                                            "Qty": nq, "Harga": nh, "Satuan": ns
                                        })
                                    
                                    st.write("---")
                                    st.write("### 2. Tambah Barang Baru")
                                    add_new = st.multiselect("Pilih dari Database:", options=df_barang['Nama Barang'].tolist(), key=f"add_{real_row_idx}")
                                    ins_pos = st.number_input("Sisip di Urutan No:", value=float(len(items_list)+1), key=f"insp_{real_row_idx}")

                                    if st.form_submit_button("💾 FIX & SIMPAN PERUBAHAN", use_container_width=True):
                                        # Gabung data lama & baru
                                        final_items = []
                                        # Barang lama
                                        for x in updated_temp:
                                            if not x["delete"]:
                                        ...
                                        for p in add_new:
                                            ...
                                            final_items.append({
                                                "pos_index": ins_pos, "Nama Barang": p, "Qty": 1, 
                                                "Harga": float(rb['Harga']), "Satuan": str(rb['Satuan'])
                                            })
                                            ins_pos += 0.01 # Agar urutan tetap terjaga jika tambah banyak
                                        
                                        # SORTING BERDASARKAN POSISI
                                        final_items = sorted(final_items, key=lambda x: x["pos_index"])
                                        
                                        # Hitung ulang Total Row & Siapkan Data
                                        save_ready = []
                                        for x in final_items:
                                            save_ready.append({
                                                "Nama Barang": x["Nama Barang"], "Qty": x["Qty"], 
                                                "Harga": x["Harga"], "Satuan": x["Satuan"], 
                                                "Total_Row": x["Qty"] * x["Harga"]
                                            })
                                        
                                        # UPDATE LANGSUNG KE GSHEET
                                        sheet.update_cell(real_row_idx, 5, str(save_ready))
                                        
                                        # BERSIHKAN SEMUA CACHE & REFRESH TOTAL
                                        st.cache_data.clear()
                                        st.success("DATA TERUPDATE DI GOOGLE SHEETS!")
                                        time.sleep(1)
                                        st.rerun()

                                # --- AREA DOWNLOAD (BACA DARI DATA TERBARU) ---
                                if items_list:
                                    ...
                                    st.divider()
                                    c_no, c_tot = st.columns([2, 1])
                                    no_s = c_no.text_input("No Surat:", value=f"/S-TTS/III/2026", key=f"no_{real_row_idx}")
                                    c_tot.metric("Grand Total", f"Rp {gtot:,.0f}")
                                    
                                    b1, b2 = st.columns(2)
                                    pdf_data = generate_pdf(no_s, row['Customer'], row['UP'], f_df, subt, tax, gtot)
                                    b1.download_button("📩 Download PDF", data=pdf_data, file_name=f"{row['Customer']}.pdf", key=f"pdf_{real_row_idx}", use_container_width=True)
                                    
                                    xls_data = generate_excel(no_s, row['Customer'], row['UP'], f_df, subt, tax, gtot)
                                    b2.download_button("📊 Download Excel", data=xls_data, file_name=f"{row['Customer']}.xlsx", key=f"xls_{real_row_idx}", use_container_width=True)

                                    if st.button("✅ Selesai", key=f"fin_{real_row_idx}", type="primary", use_container_width=True):
                                        sheet.update_cell(real_row_idx, 6, "Processed")
                                        st.rerun()
                    else: st.info("Antrean bersih, Pak!")
