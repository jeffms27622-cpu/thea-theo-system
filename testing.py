...
def validate_wa(nomor: str) -> bool:
    """Validasi nomor WA: hanya angka, panjang 9-15 digit."""
...
def get_gsheet_client():
    """Cache koneksi GSheet agar tidak buka koneksi baru setiap interaksi."""
    try:
        scope = [
...
def pesanan_to_json(items: list) -> str:
    """Simpan pesanan sebagai JSON string (bukan repr Python)."""
    return json.dumps(items, ensure_ascii=False)

def json_to_pesanan(raw: str) -> list:
    """
    Parse pesanan dari GSheet.
    Support format lama (Python list/dict string) dan format baru (JSON).
    """
    raw = raw.strip()
    if not raw:
        return []
...
def generate_excel(no_surat, nama_cust, pic, df_order, subtotal, ppn, grand_total):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for col_num, col_name in enumerate(['NO', 'DESCRIPTION', 'QTY', 'UNIT', 'PRICE', 'TOTAL']):
            worksheet.write(header_row, col_num, col_name, fmt_hdr_tbl)
...
# =========================================================
# 10. LOGIN DENGAN RATE LIMITING
# =========================================================
def check_auth(pwd: str) -> bool:
    """Cek autentikasi dengan rate limiting dan lockout."""
    now = datetime.utcnow()
...
if menu == "🏠 Home":
    ...
    if df_barang.empty:
        st.warning(
            ...
            "Silakan masuk ke **Sales Dashboard → Update Database (.csv)** untuk upload file."
        )
...
# ─────────────────────────────────────────────────────────
    ...
    if df_barang.empty:
        st.error("❌ Database barang belum tersedia. Hubungi tim sales untuk info lebih lanjut.")
        st.stop()
    ...
    with st.container(border=True):
        ...
        if pilihan_barang:
            ...
            mode_c = c1.selectbox(
                f"Pilih Satuan (Default: {satuan_db})", list_mode_cust,
            ...
            if mode_c == "Lusin (12)":
                ...
                isi_c  = c2.number_input(
                    f"Isi per {mode_c}", min_value=1, value=10,
                    key=f"isi_c_{st.session_state.widget_id}"
                )
                mult_c    = isi_c
                sat_final = mode_c

            qty_c    = c3.number_input(
                f"Jumlah {sat_final}", min_value=1, value=1,
            ...
            if st.button("➕ Masukkan ke Daftar Pesanan", use_container_width=True):
                st.session_state.cart = [
                    ...
                    if item['Nama Barang'] != pilihan_barang
                ]
    ...
    if st.session_state.cart:
        ...
        if st.button(
            f"🚀 KIRIM PENAWARAN KE PAK {MARKETING_NAME.upper()}",
            ...
            if errors:
                ...
                if sheet:
                    ...
                    sheet.append_row([
                        wkt, nama_toko, up_nama, wa_nomor,
                        pesanan_to_json(st.session_state.cart),
                        "Pending", MARKETING_NAME
                    ])
                    logger.info(f"Penawaran dikirim: {nama_toko} ({len(st.session_state.cart)} item)")
                    st.balloons()
...
# ─────────────────────────────────────────────────────────
    ...
    with st.sidebar.expander("📁 Update Database (.csv)", expanded=False):
        ...
        if up_f and st.button("🚀 Update Sekarang"):
            ...
            load_db.clear()  # Hanya clear cache DB, bukan semua cache
    ...
    try:
        ...
        logger.error(f"Gagal get_all_values: {e}")
        st.stop()
            ...
            for idx in pending.index:
                ...
                with st.expander(f"🛠️ KELOLA: {row['Customer']}", expanded=True):
                    ...
                    for i, r in enumerate(items_list):
                        ...
                        harga_master_asli = (
                            float(row_master['Harga'].values[0])
                    ...
                    if st.button("💾 SIMPAN PERUBAHAN DATA", key=f"btn_save_{idx}", use_container_width=True):
                        final = sorted(
                        ...
                        if real_row == -1:
                            st.error("❌ Gagal menemukan baris di spreadsheet. Coba refresh.")
                        else:
                            sheet.update_cell(real_row, 5, pesanan_to_json(save_data))
                            logger.info(f"Data disimpan: {row['Customer']} row={real_row}")
                    ...
                    if items_list:
                        ...
                        st.caption("💡 Klik **Simpan** dulu sebelum download agar data PDF/Excel selalu terbaru.")

                        c_no, c_met = st.columns([2, 1])
                        no_s = c_no.text_input(
                            "No Surat:", value="/S-TTS/IV/2026",
                            key=f"ns_print_{idx}"
                        )
                        ...
                        if st.button(
                            "✅ PENAWARAN SELESAI (HAPUS DARI ANTREAN)",
                            key=f"done_btn_{idx}",
                            type="primary",
                            use_container_width=True
                        ):
    ...
    with tab_riwayat:
        ...
        if processed.empty:
            ...
            if search_q:
                processed = processed[
                    processed['Customer'].str.contains(search_q, case=False, na=False)
                ]
            ...
            for idx in processed.index:
                ...
                with st.expander(f"✅ {row['Customer']} — {row['Waktu']} | Total: Rp {gtot:,.0f}"):
                    ...
                    if items_list:
                        ...
                        no_s2 = c_no2.text_input("No Surat:", value="/S-TTS/IV/2026", key=f"ns_rw_{idx}")
