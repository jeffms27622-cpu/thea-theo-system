                    ...
                    if not pending.empty:
                        for idx, row in pending.iterrows():
                            ...
                            with st.expander(f"🛠️ KELOLA: {row['Customer']} ({waktu_str})", expanded=True):
                                ...
                                # TOMBOL DOWNLOAD & PENAMAAN OTOMATIS
                                ...
                                if not final_df.empty:
                                    ...
                                    # Format Nama File: NamaCustomer - DDMMYY
                                    tgl_skrg = datetime.now().strftime('%d%m%y')
                                    nama_file_rapi_pdf = f"{row['Customer']} - {tgl_skrg}.pdf"
                                    nama_file_rapi_xls = f"{row['Customer']} - {tgl_skrg}.xlsx"
                                    
                                    b1, b2 = st.columns(2)
                                    pdf_data = generate_pdf(no_s, row['Customer'], row['UP'], final_df, subt, tax, gtot)
                                    b1.download_button("📩 PDF Presidential", data=pdf_data, file_name=nama_file_rapi_pdf, key=f"dlp_{idx}", use_container_width=True)
                                    
                                    xls_data = generate_excel(no_s, row['Customer'], row['UP'], final_df, subt, tax, gtot)
                                    b2.download_button("📊 Excel Presidential", data=xls_data, file_name=nama_file_rapi_xls, key=f"dlx_{idx}", use_container_width=True)
